...
- Organize platform-specific Rust providers as folders with a shared `mod.rs` contract and platform files such as `windows.rs` and `mac.rs` (see `capture/activity/`, `capture/input/`, `capture/screenshot/`, `capture/active_window/`, `capture/graphics_session/`, `capture/ui_automation/`). Keep Windows API calls out of shared contracts.
