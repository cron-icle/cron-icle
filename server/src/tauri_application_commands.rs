//! Application-level operations exposed over HTTP by `http_api`.
//!
...
//! work is launched in a background thread so request handlers stay
//! responsive.
...
pub struct AppState {
    ...
    /// UI (there is no push/event channel once the app runs as a plain HTTP
    /// server). `None` when no download is in flight.
    pub download_progress: Mutex<Option<crate::local_inference_setup::DownloadProgress>>,
...
impl AppState {
    ...
    /// pooled read-only connection (see `ReaderPool`) so UI queries never
...
/// the UI instead of pushed, since there is no event channel once the app
/// runs as a plain HTTP server.
...
///
/// Runs the move synchronously and returns once it's done; the caller
/// (`http_api`) is responsible for relaunching the process and exiting after
/// the HTTP response has had a chance to flush.
