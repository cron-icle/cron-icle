<#
.SYNOPSIS
    Installs the latest (or a specific) Cronicle release binary.

.DESCRIPTION
    Cronicle ships as a single unsigned binary - no installer, no
    ...
    places it under %LOCALAPPDATA%\Programs\Cronicle, and adds that folder
    to the current user's PATH so `cronicle` works from any terminal.
...
#>
param([string]$Version = "latest", [string]$InstallDir = (Join-Path $env:LOCALAPPDATA "Programs\Cronicle"))
# InstallDir deliberately isn't %LOCALAPPDATA%\Cronicle: that's a natural
# folder name for a user to also pick as Cronicle's *data* directory (event
# database, downloaded models) from Settings, and this installer must never
# write into that folder.

$ErrorActionPreference = "Stop"
$repo = "anadi45/cronicle"
$assetName = "cronicle-windows-x86_64.exe"
...
function Get-ReleaseAsset {
    ...
    $release = Invoke-RestMethod -Uri $uri -Headers @{ "User-Agent" = "cronicle-install-script" } -ErrorAction Stop
...
try {
    ...
    $exePath = Join-Path $InstallDir "cronicle.exe"

    Write-Host "Downloading Cronicle $($asset.Tag) to $exePath..."
    ...
    Write-Host ""
    Write-Host "Cronicle $($asset.Tag) installed to $exePath" -ForegroundColor Green
    Write-Host "Run it with: cronicle"
    ...
    Write-Host ""
    Write-Host "Cronicle install failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
