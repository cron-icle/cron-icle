...
use crate::persistence::writer::{millis_since_last_input, ScreenshotCache};
use crate::inference::embedding::TextEmbedder;
use crate::inference::model_provider::LlamaCppProvider;
use crate::persistence::sqlite::Database;
use crate::persistence::sqlite::SemanticEvent;
use chrono::Utc;
...
fn persist_semantic_result(
    ...
    output: crate::processing::semantic::SemanticModelOutput,
) -> Result<(), String> {
    if database
        .lock()
            ...
            TaskType::SemanticImageAnalysis => {
                ...
                let image = match cached {
                    ...
                    None => {
                        ...
                        crate::capture::graphics_session::capture_one_frame_png(
                            window_handle as isize,
                        )
                        .or_else(|_| {
                            crate::capture::active_window::capture_window_png(
                                window_handle as isize,
                            )
                        })?
                    }
    ...
    thread::spawn(move || {
        ...
        while !stop.load(Ordering::Relaxed) {
            ...
            let Some(task) = task else {
                ...
                crate::inference::native::sweep_idle_engines();
