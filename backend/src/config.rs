...
/// The local-only port Chronicle binds its HTTP server to. Overridable via
/// `CHRONICLE_PORT`, mainly so more than one instance can run side by side
/// during development.
...
pub fn should_open_browser() -> bool {
    std::env::var("CHRONICLE_NO_OPEN").is_err()
}
