...
use crate::inference::embedding::TextEmbedder;
use crate::processing::semantic::{
...
pub mod engine_paths {
    ...
    /// (see `backend/resources/llama/`) rather than downloaded at runtime,
...
impl LlamaCppProvider {
    ...
    fn generate_chat(&self, prompt: &str, max_tokens: u32) -> Result<String, String> {
        ...
        let engine = crate::inference::native::generation_engine(&chat_model, n_ctx, inference_thread_count() as i32)?;
    ...
    pub fn analyze_image(&self, bytes: &[u8]) -> Result<SemanticModelOutput, String> {
        ...
        let engine = crate::inference::native::vision_engine(
            &chat_model,
            &mmproj,
            n_ctx,
    ...
    pub fn embed_batch(&self, inputs: &[String]) -> Result<Vec<Vec<f32>>, String> {
        ...
        let n_ctx = std::num::NonZeroU32::new(SERVER_CONTEXT_SIZE).expect("SERVER_CONTEXT_SIZE is nonzero");
        let engine =
            crate::inference::native::embedding_engine(&embed_model, n_ctx, inference_thread_count() as i32)?;
