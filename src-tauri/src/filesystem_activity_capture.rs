...
fn path_is_excluded(path: &Path, excluded_paths: &[String]) -> bool {
    let candidate_components = crate::activity_capture::path_components(&path.to_string_lossy());
    excluded_paths.iter().any(|excluded| {
        let excluded = excluded.trim();
        if excluded.is_empty() {
            return false;
        }
        let excluded_components = crate::activity_capture::path_components(excluded);
        crate::activity_capture::contains_component_sequence(
